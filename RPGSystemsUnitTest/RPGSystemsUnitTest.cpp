...
namespace RPGSystemsUnitTest
{
	...
	public:
        ...
        TEST_METHOD(Player_Berserker) {
            PlayerCharacter p1(new Berserker());
            Assert::AreEqual((int)Berserker::BASEHP, (int)p1.GetMaxHP());
            Assert::AreEqual((int)Berserker::BASEEP, (int)p1.GetMaxEP());
            //no test for rage points because it has a different mechanic
            

            p1.GainEXP(100u);
            // level 2
            Assert::AreEqual(2, (int)p1.GetLevel());
            Assert::AreEqual(std::string("RageAttack"), p1.GetAbilityList().front()->GetName());
            Assert::AreEqual((int)(Berserker::BASEHP + (Berserker::BASEHP / 2.f)), (int)p1.GetMaxHP());
            Assert::AreEqual((int)(Berserker::BASEEP + (Berserker::BASEEP / 2.f)), (int)p1.GetMaxEP());
        }
        TEST_METHOD(Player_Rogue) {
            PlayerCharacter p1(new Rogue());
            Assert::AreEqual((int)Rogue::BASEHP, (int)p1.GetMaxHP());
            Assert::AreEqual((int)Rogue::BASEEP, (int)p1.GetMaxEP());
            ...
            Assert::AreEqual((int)(Rogue::BASEEP + (Rogue::BASEEP / 2.f)), (int)p1.GetMaxEP());
        }
        ...
        TEST_METHOD(Enemy_Monster) {
            ...
            Assert::AreEqual(10, (int)monster.HP.GetMax());
            Assert::AreEqual(2, (int)monster.GetMinDamage());
            Assert::AreEqual(4, (int)monster.GetMaxDamage());

            int damage_random = 0;
            for (int i = 0; i < 10; ++i) {
                damage_random = monster.Attack();

                Assert::IsTrue((damage_random >= (int)monster.GetMinDamage()));
                Assert::IsTrue((damage_random <= (int)monster.GetMaxDamage()));
            }
        }
        TEST_METHOD(Simple_Battle) {
            PlayerCharacter player(new Rogue());
            Monster monster(10, 2, 4);

            Assert::IsTrue(ItemManager::Equip(ItemManager::CreateWeapon("Dagger", CoreStats(), WEAPONSLOT::MELEE, 1, 3, false), &player));
            
            while (monster.HP.GetCurrent() > 0 && player.GetCurrentHP() > 0) {
                monster.HP.ReduceCurrent(player.MeleeAttack());
                player.TakeDamage(monster.Attack());
            }

            Assert::IsTrue((monster.HP.GetCurrent() == 0) || (player.GetCurrentHP() == 0));
        }
	};
}
