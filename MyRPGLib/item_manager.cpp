...
[[nodiscard]] Item* ItemManager::CreatePotion(std::string name, welltype Heal, itemcount quant, Buff* _buff) {
    Item* temp_item = new Item(new Potion(name, Heal, (quant == 0) ? 1 : quant, _buff));
    return temp_item;
}
...
bool ItemManager::Equip(Item* item_to_equip, PlayerCharacter* p_char) {
    ...
    if (armor) {
        ...
        if (p_char->_equipped_armor[slot_num]) {
            p_char->_equipped_armor[slot_num]->_marked_as_just_equipped = false;
            ...
            item_to_equip->_marked_as_just_equipped = true;
        }
        else {
    ...
    if (weapon) {
        ...
        if (p_char->_equipped_weapons[slot_num]) {
            p_char->_equipped_weapons[slot_num]->_marked_as_just_equipped = false;
